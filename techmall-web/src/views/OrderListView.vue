    ...
    <div class="page-container" style="margin-top: var(--space-xl); min-height: calc(100vh - 320px)">
      ...
      <div class="table-wrap">
        <el-table :data="orders" v-loading="loading">
          ...
          <el-table-column label="金额"><template #default="{row}"><span class="price-cell">¥{{ row.totalAmount }}</span></template></el-table-column>
          <el-table-column prop="status" label="状态"><template #default="{row}"><el-tag :type="statusType(row.status)">{{ row.status }}</el-tag></template></el-table-column>
          ...
          <el-table-column label="操作"><template #default="{row}">
            ...
            <el-button v-if="row.status === 'PENDING'" size="small" text type="danger" @click="cancelOrder(row.id)">取消</el-button>
